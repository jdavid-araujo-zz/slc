	...
	public Slc parse() {
		...
		try {
         ...
         sisMsg = new SisMsg(doc.getElementsByTagName("CodMsg").item(0).getTextContent(), 
        		 ...
        		 new SimpleDateFormat("yyyy-MM-dd'T'hh:mm:ss").parse(doc.getElementsByTagName("DtHrSLC").item(0).getTextContent()), 
        		 new SimpleDateFormat("yyyy-MM-dd").parse(doc.getElementsByTagName("DtMovto").item(0).getTextContent()));
         
         grupoLiquido = new GrupoLiquido(new SimpleDateFormat("yyyy-MM-dd").parse(doc.getElementsByTagName("DtLiquid").item(0).getTextContent()),
         ...
         for (int i = 0; i < nodeProdutoList.getLength(); i++) {
            ...
            if(nodeProduto.getNodeType() == Node.ELEMENT_NODE) {
                ...
                GrupoProduto grupoProduto = new GrupoProduto( elem.getElementsByTagName("CodProdt").item(0).getTextContent()); 
                ...
                for (int j = 0; j < nodeLiquidoProdutoList.getLength(); j++) {
                   ...
                   if(nodeLiquidoProduto.getNodeType() == Node.ELEMENT_NODE) {
                       ...
                       GrupoLiquidoProduto grupoLiquidoProduto = new GrupoLiquidoProduto( elem1.getElementsByTagName("IdentdLinhaBilat").item(k).getTextContent(),
                    		   ...
                    		   Long.parseLong(elem1.getElementsByTagName("TpTranscSLC").item(k).getTextContent())); 
